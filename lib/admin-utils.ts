...
export const ADMIN_ROUTE_PERMISSIONS = {
  '/admin/dashboard': ADMIN_ROLES, // All admins
  '/admin/users': ['SUPER_ADMIN', 'QUALITY_ADMIN'],
  '/admin/talent-review': ['QUALITY_ADMIN', 'SUPER_ADMIN'],
  '/admin/jobs': ['QUALITY_ADMIN', 'SUPER_ADMIN'],
...
export function hasRouteAccess(userRole: string, route: string): boolean {
  ...
  for (const [routePattern, allowedRoles] of Object.entries(ADMIN_ROUTE_PERMISSIONS)) {
    if (route.startsWith(routePattern)) {
      return (allowedRoles as readonly AdminRole[]).includes(userRole as AdminRole)
    }
  }
