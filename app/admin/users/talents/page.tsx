...
export default async function TalentUsersPage() {
  ...
  // Count total talent users from database
  const [
    { count: totalTalents },
  ...
  if (profilesError) {
    ...
    users = profilesData || []
    console.log('Fetched talent users from profiles table:', users.length, 'users')
  }

  // Fetch talent profiles for additional data
  const userIds = users.map((u: any) => u.id)
  const { data: talentProfiles } = userIds.length > 0
    ? await adminSupabase
        .from('talent_profiles')
        .select('user_id, hourly_rate, profile_completion, is_profile_ready')
        .in('user_id', userIds)
    : { data: [] }

  const talentProfileMap = new Map(
    (talentProfiles || []).map((tp: any) => [tp.user_id, tp])
  )
  ...
  const profilesWithEmail = await Promise.all(
    users.map(async (user: any) => {
      ...
      const talentProfile = talentProfileMap.get(user.id)
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-4xl font-bold mb-2">Talent User Management</h1>
        <p className="text-gray-600">View and manage talent accounts</p>
      </div>

      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <CardTitle>Talent Users ({totalTalents || 0})</CardTitle>
            <CreateUserButton defaultRole="TALENT" />
          </div>
        </CardHeader>
        <CardContent>
          {error ? (
            ...
            <UsersTable users={profiles || []} userType="talent" canDelete={adminRole === 'SUPER_ADMIN'} />
          )}
        </CardContent>
      </Card>
    </div>
